...
export class LatencyComponent implements OnInit, OnDestroy {
  ...
  // sample measures steady-state latency rather than handshake cost (issue #197).
  private readonly warmedAccounts = new Set<string>()
  ...
  private async pingRegion(region: RegionPingData): Promise<void> {
    ...
    // dwarfs the actual round-trip and made local regions appear slower than
    // distant ones. The warm-up establishes a keep-alive connection so every
    // timed sample reflects steady-state latency. See issue #197.
    if (!this.warmedAccounts.has(region.storageAccountName)) {
      this.warmedAccounts.add(region.storageAccountName)
      await this.sendPing(url)
    }

    const startTime = performance.now()
    const response = await this.sendPing(url)
  ...
  private async sendPing(url: string): Promise<Response | null> {
    const controller = new AbortController()
    const timeoutId = setTimeout(() => controller.abort(), LatencyComponent.CONFIG.PING_TIMEOUT_MS)

    try {
      // The unique `?_=` query string already defeats caching, so we must NOT
      // set `cache: 'no-cache'`: that header (Cache-Control) is not CORS-safelisted
      // and turns each HEAD into a non-simple request, triggering a preflight
      // OPTIONS round-trip on every ping (the cache-buster also defeats the
      // preflight cache), roughly doubling measured latency. See issue #197.
      return await fetch(`${url}?_=${Date.now()}`, {
        method: 'HEAD',
  ...
  private queuePingUpdate(storageAccountName: string, latency: number): void {
    ...
    // (e.g. South Africa North at ~600 ms), making them vanish from the results
    // entirely instead of showing their true latency. See issue #197.
